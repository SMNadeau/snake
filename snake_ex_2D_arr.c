...
void kill_snake();
int visited_check(int visited_array[][1], int move, int col, int row);
...
int main()
{
    ...
    int visited_array[COLS*LINES][1];
...
int visited_check(int visited_array[][1], int move, int col, int row) //returns 1 on visited
{
    for(int i = 0; i <= move; i++)  //look at every move that's been made so far
    {
        if(visited_array[i][0] == col) //if the col matches one already in the list 
        {
            if(visited_array[i][1] == row) //if the respective row for that matching column also matches
